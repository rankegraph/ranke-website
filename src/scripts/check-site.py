...
# The built site, which is what a reader gets. `make site` writes it.
...
def error(page: Path, message: str) -> None:
    errors.append(f"{page.relative_to(DIST.parent)}: {message}")


def warn(page: Path, message: str) -> None:
    warnings.append(f"{page.relative_to(DIST.parent)}: {message}")
...
# A page kept at its old URL emits a redirect stub, which carries none of what a
# page carries and is not one: it exists to send a reader on.
...
def read_pages() -> dict[Path, Page]:
    ...
    for path in sorted(DIST.rglob("*.html")):
        if REDIRECT in path.read_text(encoding="utf-8"):
            redirects += 1
            continue
        parser = Page()
        ...
        pages[path] = parser
    if redirects:
        print(f"   ({redirects} redirect(s) skipped)")
    return pages
...
def check_pages(pages: dict[Path, Page]) -> None:
    ...
    The pages written by hand share one head down to the order of its lines,
    and the docs section's generated pages carry the same one. A page that omits
    the canonical link or the description is the likely mistake, and neither
    ...
    for path, page in pages.items():
        ...
        # The canonical URL is derivable from where the page sits, so a copied
        # page that kept its source's canonical is a mistake a machine catches.
        # A directory's index.html is served as the directory itself.
        rel = path.relative_to(DIST)
        if rel.name == "index.html":
            parent = rel.parent.as_posix()
            want = f"{SITE}/" if parent == "." else f"{SITE}/{parent}/"
        else:
            want = f"{SITE}/{rel.as_posix()}"
...
def check_links(pages: dict[Path, Page]) -> None:
    ...
    for path, page in pages.items():
        for value, line in page.links:
            ...
            if target:
                # A leading slash is the site root, and a directory is served as
                # the index.html inside it — both are what the host does.
                if target.startswith("/"):
                    dest = (DIST / target.lstrip("/")).resolve()
                else:
...
def check_classes(pages: dict[Path, Page]) -> None:
    ...
    for name in sorted(defined - set(used)):
        warn(DIST / "style.css", f'.{name} is defined and unused')
...
def main() -> int:
    ...
    pages = read_pages()
    if not pages:
        print(f"check-site: no pages under {DIST} — run `make site` first", file=sys.stderr)
        return 1
